...
describe('authGuard', () => {
  ...
  it('allows navigation when authenticated', () => {
    ...
    const result = runGuard('/diretoria');
  ...
  it('redirects to /login without a reason when there was no prior session', () => {
    ...
    const result = runGuard('/diretoria') as UrlTree;

    expect(result instanceof UrlTree).toBe(true);
    expect(router.serializeUrl(result)).toBe('/login?returnUrl=%2Fdiretoria');
  });
  ...
  it('redirects to /login with reason=expired when a stored token was dropped as expired', () => {
    ...
    expect(serialized).toContain('returnUrl=%2Fdiretoria');
  });
});
