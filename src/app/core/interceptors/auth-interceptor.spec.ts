...
describe('authInterceptor', () => {
  ...
  it('attaches the bearer token to API requests when authenticated', () => {
    ...
    http.get(`${environment.apiUrl}/diretoria/todos`).subscribe();

    const req = httpMock.expectOne(`${environment.apiUrl}/diretoria/todos`);
  ...
  it('logs out and redirects with reason=expired on a 401 for an authenticated request', () => {
    ...
    http.get(`${environment.apiUrl}/diretoria/todos`).subscribe({ error: () => {} });

    httpMock.expectOne(`${environment.apiUrl}/diretoria/todos`).flush(
