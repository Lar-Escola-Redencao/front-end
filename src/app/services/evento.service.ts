...
export class EventoService {
  ...
  criar(evento: CriarEventoDTO): Observable<Evento> {
    ...
    formData.append('dataEvento', evento.dataEvento.toISOString());
    formData.append('endereco', evento.endereco);
    if (evento.imagem) formData.append('imagem', evento.imagem);
    if (evento.valor) formData.append('valor', evento.valor.toString());
    formData.append('tipoEvento', evento.tipoEvento);
    if (evento.parceirosIds) {
  ...
  atualizar(id: number, evento: AtualizarEventoDTO): Observable<Evento> {
    ...
    if (evento.dataEvento) formData.append('dataEvento', evento.dataEvento.toISOString());
    ...
    return this.http.put<Evento>(`${this.apiUrl}/atualizar/${id}`, formData);
  }
